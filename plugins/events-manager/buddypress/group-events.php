...
//entered to allow request to reuturn to this page so as to load templates within BP
$this_url = $bp->canonical_stack['canonical_url'];
...
$this_url = $parse['scheme'].'://'.$parse['host'].$parse['path'];
	...
	?>
	<!-- load event edit form -->
	<?php if ($action == 'edit' && $event_id) {
		em_locate_template('forms/event-editor.php',true, array('args'=>$args));
	}
	elseif ($action == 'show_event' && $event_id) {
		em_locate_template('templates/single-group-event.php',true, array('args'=>$args));
	}
	elseif ($action == ('add_result' || 'modify_list' || 'eliminate')) {
		locate_template('pkr_templates/add-results.php',true);
	}

	?>

	<!-- Hide the tablenav if editing event, UNLESS it's showing the success message -->
	<?php if (($action == 'edit' || $action == 'show_event') && ($_REQUEST['success'] != 1)) {
	...
	<div class="tablenav" <?php echo $hideme ?>>
	...
	if (empty ( $EM_Events )) {
		...
		echo "<p>". __( 'No scheduled games','dbem' ) ."</p>";
		...
		<table class="widefat events-table" <?php echo $hideme ?>>
			<thead>
				<tr>
						...
						*/ ?>
						...
						<td>
							...
							<strong>
								...
								<a href="<?php echo $this_url; ?>?action=show_event&amp;event_id=<?php echo $event->event_id ?>"><?php echo $event->output('#_EVENTNAME'); ?></a>								
							...
							<div class="row-actions">
								...
								<a href="<?php echo $this_url ?>?action=edit&amp;event_id=<?php echo $event->event_id ?>"><?php _e('Edit', 'dbem'); ?></a>
